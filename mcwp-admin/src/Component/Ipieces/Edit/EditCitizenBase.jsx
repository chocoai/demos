 ...
 */
class EditCitizenBase extends Component {
    constructor(props) {
        super(props);
        this.state = {
            pictureInfo: '',
    ...
    getPictureInfo = (type) => {  // 照片信息
        ...
        Config.get('/v1/oss/' + code + fileType + '/*', {}, (res) => {
            if (res.code == Config.errorCode.success) {
                ...
                this.setState({
                    pictureInfo: [...res.data[type[0]],...res.data[type[1]]||[]],
                    preview: true
                });
            } else {
                ...
                <Form>
                    <div className='ipieces-subtitle-container'>
                        ...
                        <div>
                            ...
                            <span className='ipieces-subtitle-attachment' onClick={() => this.getPictureInfo([Config.bizType.loanPerson,Config.bizType.loanCreditFamily])}>查看文件</span>
                        </div>
                    </div>
                    <Row className="personal-info-content" type="flex" justify="start">
                        ...
                        <Col span={12} className="subtitle">
                            <span>月收入：{loanCustomer.monthIncome && loanCustomer.monthIncome + '元' || '未录入'}</span>
                        </Col>
                        <Col span={12} className="subtitle">
                            <span>当前月还贷金额：{loanCustomer.monthDebt && loanCustomer.monthDebt + '元' || '未录入'}</span>
                        </Col>
                        <Col span={12} className="subtitle">
                            <span>信用卡汇总额度：{loanCustomer.creditcardSumamt && loanCustomer.creditcardSumamt + '元' || '未录入'}</span>
                        </Col>
                        <Col span={12} className="subtitle">
...
const pureEditCitizenBase = pureRender(EditCitizenBase);

export default pureEditCitizenBase;
