...
/**
 * 系统配置 —— 现场调查表配置 目前和申请表区别不大，如果未来区别保持不变，可以考虑合并
 * @Author: 赵俊
 * @Date:   2018-03-07
 * @Last Modified by:   赵俊
 * @Last Modified time: 2018-03-07
 */
class FormSiteConf extends Component {
    constructor(props) {
        super(props);
        this.state = {
            formType: 3,      // 1申请表2后台调查表3现场调查表
            formConf: [],
    ...
    render() {
        ...
        const columns = [{
            ...
            render: (text, record, index) => (
                <span>
                    <Link className="form-link" to={`/form/site/${record.formEnName}/${prdType}`}>配置</Link>
                </span>
            )
        }];
        const bcrumb = [{
