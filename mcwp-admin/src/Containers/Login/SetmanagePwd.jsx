...
// 该组件待封装
class SetManagePwd extends Component {
	constructor(props) {
		super(props);
		this.state = {
			password: '',
	...
	handleSubmit = (e) => {
		...
		let setPwdParams = passParams + userInfo; 
		setPwdParams = Object.assign(userInfo, passParams)
							
		Config.post('/comm/userEnterpr', setPwdParams, (res) => {
			if(res.code == 0) {
	...
	componentDidMount() {
		...
		Config.get('/comm/userEnterpr/isExpire', {token: token}, (res) => {   // 判断邮件urltoken是否失效
            if(res.code == 0) {
            	this.setState({
					loginSpinning: false,
					userInfo: res.data
					...
					<div className="login-frame">
						<Spin tip="载入中..." spinning={this.state.loginSpinning}>
							<div className="form-title">设置密码</div>
								...
								<Input className='repPassword' size="large" type="password" placeholder="请再次输入您的新密码" onChange={this.checkRepPassword} />
								...
								<Button onClick={this.handleSubmit} className='common-large-btn password-reset' type="primary" htmlType="submit" loading={this.state.loginBtnLoading}>{this.state.loginBtnText}</Button>
						</Spin>
					</div>
		    	</Content>
