...
class RuleIndex extends Component {
	...
	async getDefaultValue() {
		let res = await RuleService.getDefaultValue()
		if (res.code == Config.errorCode.success) {
	...
	getRuleDetail(params) { // 行业规则详情
		...
		RuleService.getRuleDetail(params, (res) => {
			if (res.code == Config.errorCode.success) {
				const data = res.data;
				let newData = data.loanLogicVerifyDts;
				...
				this.setState({
					loading: false,
					ruleDetail: data.loanLogicVerifyDts,
					indValue: data.industryId,
					industryArr: [data.industryId],
					rightLegal: newData[0],
					grossProfit1: newData[1],
					grossProfit2: newData[2],
					grossProfit3: newData[3],
	...
	handleOkSen = () => { //提交数据
		let { procode, ruleData, rightLegal, grossProfit1, grossProfit2, grossProfit3, salesVolume1, salesVolume2, salesVolume3, salesVolume4, salesVolume5, salesVolume6, salesVolume7, salesVolume8, salesVolume9, Purchases1, Purchases2, industryArr } = this.state;
		ruleData = [];
		if (industryArr.length == 0) return message.error('请选择行业');
		var industryId = industryArr[industryArr.length - 1];
	...
	toRuleDetail(classfily, types, code) {
		const { procode, type } = this.state;
		if (code) {
			window.open(`/rule/bank/${classfily}/${types}/${code}?code=${procode}&type=${type}`, '_blank')
		} else {
			window.open(`/rule/bank/${classfily}/${types}?code=${procode}&type=${type}`, '_blank')
		}
	}
	render() {
		let { loading, ruleDetail, bcrumbValue, jylbItems, indValue, industryArr, procode, code, type, cascaderItem } = this.state;
		let treeData = [];
		...
		const bcrumb = [{
			...
			'link': '/rule/logic?code=' + procode + '&type=' + type,
			'value': '逻辑校验规则'
		}, {
			'link': null,
			'value': bcrumbValue
					...
					<div className="rule-content common-console-container">
						...
						<Row className="row-title">
							...
							<Col span={3} className="rule-action">
								{
									code ? <span className="rule-view" onClick={() => this.toRuleDetail('right', 'edit', code)}><img alt="文件详情" src={require('../../Assets/Images/icon_rule.png')} className='rule-icon' />查看规则</span> :
										<span className="rule-view" onClick={() => this.toRuleDetail('right', 'add', '')}><img alt="文件详情" src={require('../../Assets/Images/icon_rule.png')} className='rule-icon' />查看规则</span>
								}
							</Col>
						</Row>
						...
						<Row className="row-title">
							...
							<Col span={3} className="rule-action">
								{
									code ? <span className="rule-view" onClick={() => this.toRuleDetail('gross', 'edit', code)}><img alt="文件详情" src={require('../../Assets/Images/icon_rule.png')} className='rule-icon' />查看规则</span> :
										<span className="rule-view" onClick={() => this.toRuleDetail('gross', 'add', '')}><img alt="文件详情" src={require('../../Assets/Images/icon_rule.png')} className='rule-icon' />查看规则</span>
								}
							</Col>
						</Row>
						<Row className="row-content">
						...
						<Row className="row-title">
							...
							<Col span={3} className="rule-action">
								{
									code ? <span className="rule-view" onClick={() => this.toRuleDetail('sale', 'edit', code)}><img alt="文件详情" src={require('../../Assets/Images/icon_rule.png')} className='rule-icon' />查看规则</span> :
										<span className="rule-view" onClick={() => this.toRuleDetail('sale', 'add', '')}><img alt="文件详情" src={require('../../Assets/Images/icon_rule.png')} className='rule-icon' />查看规则</span>
								}
							</Col>
						</Row>
						<Row className="row-content">
						...
						<Row className="row-title">
							...
							<Col span={3} className="rule-action">
								{
									code ? <span className="rule-view" onClick={() => this.toRuleDetail('purchase', 'edit', code)}><img alt="文件详情" src={require('../../Assets/Images/icon_rule.png')} className='rule-icon' />查看规则</span> :
										<span className="rule-view" onClick={() => this.toRuleDetail('purchase', 'add', '')}><img alt="文件详情" src={require('../../Assets/Images/icon_rule.png')} className='rule-icon' />查看规则</span>
								}
							</Col>
						</Row>
						<Row className="row-content">
