<template>
    <div class="login-container" @touchmove.prevent.stop="1">
        ...
        <p class="login-skip" @click="$emit('changeTab')" v-if="showNext">跳过</p>
    </div>
</template>
...
import { wxBinding, sendWxVerifyCode } from "../../service/home.js";
import prefixUrl from "@/utils/mixins/prefixUrl";
import { getBindingUserInfo } from "../../service/common";
...
export default {
  ...
  methods: {
    ...
    async login() {
      ...
      if (!isTelephone(phone)) return that.$toast(Config.toast.errorPhone);
      if (!verificationCode)
        return that.$toast(Config.toast.nullverificationCode);
      ...
      // TODO: 3.1 调整 wap也要加
      let bindRes = await getBindingUserInfo();
      let bindInfo = (bindRes && bindRes.data) || {};
      Store.set(
        Config.constants.wxUserInfo,
        Object.assign(Store.get(Config.constants.wxUserInfo) || {}, bindInfo)
      );
      that.$emit("success");
      // this.getInfo();
    }
